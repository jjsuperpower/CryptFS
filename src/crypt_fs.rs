...
impl CryptFS {
    ...
    /// The key is hashed using SHA256 and the first 16 or 32 bytes are used as the encryption key
    /// depending whether using 128 or 256 bit encryption.
    /// This allows the user to have a password of an arbitrary length.
    /// 
    /// # Arguments
    ...
    /// Simple wrapper to compute the SHA256 HMAC
    /// # Arguments
    ...
    /// * `mode` - Whether the file will be encrypted or decrypted
    ...
    /// [`CryptFSError::InvalidFileSize`] - If the file size is less than [`HEADER_SIZE`]
    fn get_crypt_read_size(&self, file: &fs::File, mode: CryptMode) -> Result<u64, CryptFSError> {
        ...
        match mode {
            CryptMode::Encrypt => {
                ...
                let new_size = HEADER_SIZE as u64 + file_size + aes_padding;
                return Ok(new_size);
            },
            CryptMode::Decrypt => {
    ...
    /// * A [`CryptFSHeader`] struct containing decrypted header information
    /// 
    /// # Errors
    /// [`CryptFSError`]
